...
def upload_transaction_stats(time_from, time_to):
    ...
    while current_time <= end_time:
        ...
        database["stats"].insert_one(transaction_data)
...
def update_transaction_stats(date):
    ...
    transaction_count = 0
    time_now = time.time()
    query = database["stats"].find_one({ "date": date }, {"_id": 0, "last_time": 1})
...
def upload_hash_rates(time_from, time_to):
    current_time = datetime.datetime.strptime(time_from, "%Y-%m-%d").replace(tzinfo=datetime.timezone.utc).timestamp()
    end_time = datetime.datetime.strptime(time_to, "%Y-%m-%d").replace(tzinfo=datetime.timezone.utc).timestamp()
    while current_time <= end_time:
        hash_rate = get_hash_rate(24, current_time)
        date = datetime.datetime.utcfromtimestamp(current_time).strftime('%Y-%m-%d')
        database["stats"].update_one( {"date": date}, { "$set": {"hash_rate": hash_rate}} )
        print("Inserted hash rate for " + date + " value: " + str(hash_rate))
        current_time += 86400
...
#upload_transaction_stats("2009-01-06", "2022-03-21")
#upload_hash_rates("2009-01-06", "2022-03-21")
